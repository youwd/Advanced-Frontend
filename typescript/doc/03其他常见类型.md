# Typescript 中其他常见类型
原文：https://juejin.im/book/5da08714518825520e6bb810/section/5da0876df265da5b827d7fb2
...
```
## `never`
`never` 类型表示的是**那些永不存在的值**的类型，`never` 类型是任何类型的子类型，也可以赋值给任何类型；然而，没有类型是 `never` 的子类型或可以赋值给 `never` 类型（除了`never`本身之外）。
